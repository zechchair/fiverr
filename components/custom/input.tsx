    ...
    <div className={Props.className}>
        ...
        ? Props.labels.map((lbl, i) => (
            <>
              ...
              >
                ...
                />
                <label htmlFor={Props.id + lbl[Props.label]}>
                  {lbl[Props.label]}
                </label>
              </div>
            </>
        ...
        <>
          ...
          <datalist id={Props.name + "1"}>
            ...
            {Props?.labels?.map((lbl, indexl) => (
              <option key={indexl} value={lbl[Props.label]} />
            ))}
          </datalist>
        </>
      ) : null}
      {Props.type == "select" ? (
        <>
          <select
            className={
              "w-full text-sm  bg-white  py-2 pl-4 pr-2 focus:ring-1 focus:outline-none focus:border-blue-300 focus:ring-gray-200 border border-gray-200 rounded-md inline"
            +(!Props.value ? " text-gray-500 ":" text-black ")
			}
            name={Props.name}
            id={Props.name}
            onChange={(e) => {
            ...
            required={Props.required}
          >
            <option value="" disabled hidden selected={!Props.value}>
              {Props.placeholder ?? "veuillez choisir ..."}
            </option>
            {Props?.labels?.map((lbl, index) => (
              <option
                key={index}
                value={lbl.value}
                selected={Props.value == lbl.value}
              >
                {lbl.name}
              </option>
            ))}
	  ...
	  <span className="text-xs text-red-600 px-3">{Props.error}</span>
    </div>
  );
}
